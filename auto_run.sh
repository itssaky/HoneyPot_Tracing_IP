...
# Allow Python to bind to privileged ports (optional for lower ports)
sudo setcap 'cap_net_bind_service=+ep' $(which python3)

# Allow all incoming connections (if needed for honeypot)
sudo ufw allow 8080/tcp

# Create a directory for the honeypot
mkdir -p ~/honeypot
cd ~/honeypot
...
SMTP_PORT = 587
SMTP_USERNAME = "akyraj113@gmail.com"
SMTP_PASSWORD = "your_password"

def send_alert(ip, port):
    subject = "Honeypot Alert: Unauthorized Access Attempt"
    body = f"Unauthorized access detected from {ip}:{port} on {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    msg = MIMEText(body)
    msg['Subject'] = subject
